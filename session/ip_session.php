<?php 
    ...
    $session = "SELECT * FROM sessions where user_id='$user_id'";
    $session_result = $connect4->query($session);			
    $session_row = mysqli_fetch_object($session_result);
    
    //내부 ip를 획득한 단말기는 예외 처리
    if($cut_ip!="192.168.0" and $cut_ip!="192.168.1" and $cut_ip!="192.168.2" and $cut_ip!="192.168.3" and $cut_ip!="192.168.4" and $cut_ip!="192.168.5") {
        if($session_row->level!='kjwt' and $session_row->level!='master' and $session_row->level!='vietnam' and $session_row->level!='seetech') {
            Confirm();     
