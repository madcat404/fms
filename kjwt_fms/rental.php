<?php 
    ...
    // Last Modified: <Current Date> - Mobile UI Optimization (ID Conflict Fixed)
	// =============================================
    include 'rental_status.php'; 

    // [추가] 검색 결과를 배열로 저장 (모바일/데스크톱 뷰 모두 사용하기 위함)
    $search_data = [];
                                        ...
                                        <div class="tab-pane fade <?php echo $tab2_text;?>" id="tab2" role="tabpanel" aria-labelledby="tab-two">
                                            ...
                                            <div class="card shadow mb-2">
                                                ...
                                                <div class="collapse show" id="collapseCardExample25">
                                                    <div class="card-body p-2">
                                                        <?php
                                                            // 현황 데이터를 배열로 정리하여 반복문 사용 (모바일 뷰 생성을 위해)
                                                            $status_cars = [
                                                                [
                                                                    'name' => 'avante 225하 7206', 'fuel' => 'LPG',
                                                                    'info' => $Data_CarInfo6, 'prev_info' => $Data_CarInfo66, 'gw' => $gw_title6 ?? '-', 'loc' => false
                                                                ],
                                                                [
                                                                    'name' => 'avante 225하 7207', 'fuel' => 'LPG',
                                                                    'info' => $Data_CarInfo7, 'prev_info' => $Data_CarInfo77, 'gw' => $gw_title7 ?? '-', 'loc' => true
                                                                ],
                                                                [
                                                                    'name' => 'Staria 169하 9285', 'fuel' => '경유',
                                                                    'info' => $Data_CarInfo2, 'prev_info' => $Data_CarInfo22, 'gw' => $gw_title2 ?? '-', 'loc' => false
                                                                ],
                                                            ];
                                                        ?>
                                                        ...
                                                        <div class="d-md-none">
                                                            <?php foreach($status_cars as $car): 
                                                                ...
                                                                $cond_class = ($condition == '사용가능') ? 'text-success' : 'text-danger';
                                                            ?>
                                                            <div class="card mb-2">
                                                                <div class="card-body p-3">
                                                                    <div class="h6 font-weight-bold <?= $cond_class ?> text-uppercase mb-2">
                                                                        <?= $car['name'] ?>
                                                                    </div>
                                                                    <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">상태</div><div class="col-8 small <?= $cond_class ?>"><?= htmlspecialchars($condition) ?></div></div>
                                                                    <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">대여일</div><div class="col-8 small"><?= htmlspecialchars($info->SEARCH_DATE ?? '-') ?></div></div>
                                                                    <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">운전자</div><div class="col-8 small"><?= htmlspecialchars($info->DRIVER ?? '-') ?></div></div>
                                                                    <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">목적지</div><div class="col-8 small"><?= htmlspecialchars($info->DESTINATION ?? '-') ?></div></div>
                                                                    <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">KM(이전)</div><div class="col-8 small"><?= htmlspecialchars($info->END_KM ?? '-') ?> (<?= htmlspecialchars($prev->END_KM ?? '-') ?>)</div></div>
                                                                    <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">잔액(이전)</div><div class="col-8 small"><?= htmlspecialchars($info->HI_PASS ?? '-') ?> (<?= htmlspecialchars($prev->HI_PASS ?? '-') ?>)</div></div>
                                                                    <?php if($car['loc']): ?>
                                                                        <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">위치</div><div class="col-8 small"><a href="https://fms.iwin.kr/files/<?php echo htmlspecialchars($Data_CarLocation->FILE_NM ?? ''); ?>" class="btn btn-sm btn-info py-0">보기</a></div></div>
                                                                    <?php endif; ?>
                                                                    <div class="row mb-0"><div class="col-4 small font-weight-bold text-gray-600">특이사항</div><div class="col-8 small text-truncate"><?= htmlspecialchars($info->NOTE ?? '-') ?></div></div>
                                                                </div>
                                                            </div>
                                                            <?php endforeach; ?>
                                                        </div>
                                                            ...
                                                            <table class="table table-bordered table-striped" id="table2">
                                                                ...
                                                                <tbody>                                                             
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td><?php if($car['loc']): ?><a href="https://fms.iwin.kr/files/<?php echo htmlspecialchars($Data_CarLocation->FILE_NM ?? ''); ?>" class="btn btn-sm btn-info">보기</a><?php else: echo "-"; endif; ?></td>
                                                            ...
                                                            <div class="card mb-2">
                                                                <div class="card-body p-3">
                                                                    <div class="h6 font-weight-bold text-secondary text-uppercase mb-2">
                                                                        <?= htmlspecialchars($row['CAR_NM']) ?>
                                                                    </div>
                                                                    <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">대여일</div><div class="col-8 small"><?= htmlspecialchars($row['START_TIME']) ?></div></div>
                                                                    <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">반납일</div><div class="col-8 small"><?= empty($row['END_TIME']) ? '-' : htmlspecialchars($row['END_TIME']) ?></div></div>
                                                                    <div class="row mb-1"><div class="col-4 small font-weight-bold text-gray-600">운전자</div><div class="col-8 small"><?= htmlspecialchars($row['DRIVER']) ?></div></div>
                                                                    <div class="row mb-0"><div class="col-4 small font-weight-bold text-gray-600">최종KM</div><div class="col-8 small"><?= htmlspecialchars($row['END_KM']) ?></div></div>
                                                                </div>
                                                            </div>
                                                            <?php endforeach; ?>
                                                            <?php if(empty($search_data) && isset($Result_SearchCar)): ?>
                                                                <div class="text-center p-3">검색 결과가 없습니다.</div>
                                                            <?php endif; ?>
                                                        </div>
